...
/**
 * Filter bar panel mapping B2B invoice parameters.
 * @component
 */
export default function InvoiceSearch() {
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      ...
      searchInvoices(localQuery);
    }, 450);

    return () => clearTimeout(handler);
  }, [localQuery, setSearchQuery, searchInvoices]);
    ...
    <div className="filter-bar" style={{ display: "flex", flexDirection: "column", gap: "12px", padding: "16px", background: "var(--card-bg)", borderRadius: "8px", border: "1px solid var(--border)" }}>
      <div style={{ display: "flex", flexWrap: "wrap", gap: "12px", alignItems: "center" }}>
        ...
        <div style={{ flex: "1 1 250px" }}>
          <input
            ...
            placeholder="Search by invoice number, order or customer name..."
            value={localQuery}
            onChange={(e) => setLocalQuery(e.target.value)}
            aria-label="Search by invoice number, order number or customer name"
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "16px", alignItems: "center", borderTop: "1px solid var(--border)", paddingTop: "10px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          <span style={{ fontSize: "0.8rem", color: "var(--text-light)" }}>Invoice Status:</span>
          <select
            className="form-select"
          ...
          >
            ...
            {["Draft", "Issued", "Partially Paid", "Paid", "Cancelled"].map((s) => (
